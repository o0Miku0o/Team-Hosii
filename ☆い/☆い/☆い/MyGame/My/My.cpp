...
void Font::Draw(const Point * const pos_, const char * const text_)
{
	...
	HGDIOBJ old = nullptr;
	if (hf) old = SelectObject(Rec::off, hf);
	//文字色を引数から指定
	SetTextColor(Rec::off, col);
	//背景色を透過に指定
	SetBkMode(Rec::off, TRANSPARENT);
	//引数から指定されたデバイスコンテキストに文字列を描画
	TextOut(Rec::off, dp.x, dp.y, text_, lstrlen(text_) + 1);
	if (old) SelectObject(Rec::off, old);
}
...
{
#ifdef _DEBUG
	if (!mybitmap_->hBmpDC || !mybitmap_->hBmp || !mybitmap_->hMaskBmpDC || !mybitmap_->hMaskBmp) return;
#endif
	...
	else PlgBlt(off, dp, mybitmap_->hBmpDC, 0, 0, mybitmap_->BmpInfo.bmWidth, mybitmap_->BmpInfo.bmHeight, mybitmap_->hMaskBmp, 0, 0);
...
void Rec::DrawAlpha(Image * const mybitmap_, byte alpha_)
{
	...
	AlphaBlend(off, dp.x, dp.y, (int)w, (int)h, mybitmap_->hBmpDC, 0, 0, mybitmap_->BmpInfo.bmWidth, mybitmap_->BmpInfo.bmHeight, bfu);
}
//矩形の外枠を描画するメンバ関数
...
/*コンストラクタ*/
Circle::Circle()
	: hPen(nullptr), color(RGB(255, 255, 255)), center({ 0.f,0.f }), radius(1.f)
{
...
void Circle::Draw()
{
	...
	/*offがペンを使うようにする*/
	HGDIOBJ old = SelectObject(Rec::off, hPen);
	/*円の描画開始*/
	MoveToEx(Rec::off, int(dp.x + cos(DtoR(-1.f)) * radius), int(dp.y + sin(DtoR(-1.f)) * radius), nullptr);
	for (int i = 0; i < 360; ++i)
	{
		LineTo(Rec::off, int(dp.x + cos(DtoR((float)i)) * radius), int(dp.y + sin(DtoR((float)i)) * radius));
	}
	/*offが使う描画オブジェクトを元に戻す*/
	SelectObject(Rec::off, old);
	/*ペンを破棄*/
	DeleteObject(hPen);
	hPen = nullptr;
}
...
{
	return RtoD(atan2(rec_->p[CENTER].y - spos.y, rec_->p[CENTER].x - spos.x));
}
/*角度取得*/
...
void Line::Draw()
{
	...
	MoveToEx(Rec::off, (int)pS.x, (int)pS.y, nullptr);
	LineTo(Rec::off, (int)pE.x, (int)pE.y);
...
bool Line::CheckHit(const Rec * const rec_)
{
	return
		(
			LineCheckCross(spos.x, spos.y, epos.x, epos.y, rec_->p[TOP_LEFT].x, rec_->p[TOP_LEFT].y, rec_->p[TOP_RIGHT].x, rec_->p[TOP_RIGHT].y) ||
			LineCheckCross(spos.x, spos.y, epos.x, epos.y, rec_->p[TOP_RIGHT].x, rec_->p[TOP_RIGHT].y, rec_->p[BOTTOM_RIGHT].x, rec_->p[BOTTOM_RIGHT].y) ||
			LineCheckCross(spos.x, spos.y, epos.x, epos.y, rec_->p[BOTTOM_RIGHT].x, rec_->p[BOTTOM_RIGHT].y, rec_->p[BOTTOM_LEFT].x, rec_->p[BOTTOM_LEFT].y) ||
			LineCheckCross(spos.x, spos.y, epos.x, epos.y, rec_->p[BOTTOM_LEFT].x, rec_->p[BOTTOM_LEFT].y, rec_->p[TOP_LEFT].x, rec_->p[TOP_LEFT].y)
			);
}
/*当たり判定*/
